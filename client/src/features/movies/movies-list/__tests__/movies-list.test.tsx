...
describe('MoviesList', () => {
  ...
  it('should render skeleton cards when loading', () => {
    ...
    // Should not render actual movies when loading
  ...
  it('should render empty grid when no movies and not loading', () => {
    ...
    // Should render grid container but no movie cards
    const gridContainer = screen.getByRole('grid') || document.querySelector('.MuiGrid-container')
  ...
  it('should handle undefined movies prop', () => {
    ...
    // Should not crash and should not render any movie content
    expect(screen.queryByText('Test Movie 1')).not.toBeInTheDocument()
  })
  ...
  it('should render correct number of skeleton cards when loading', () => {
    ...
    // Should render exactly 20 skeleton cards as per the component logic
  ...
  it('should render movies in grid layout', () => {
    const { container } = render(<MoviesList {...defaultProps} />)
    
    const gridContainer = container.querySelector('.MuiGrid-container')
    expect(gridContainer).toBeInTheDocument()
    
    const gridItems = container.querySelectorAll('.MuiGrid-item')
    expect(gridItems.length).toBe(mockMovies.length)
  })

  it('should pass isMobile prop to movie cards', () => {
    render(<MoviesList {...defaultProps} isMobile={true} />)
    
    // The isMobile prop should be passed to each MoviesCard component
    // This is tested indirectly by ensuring the movies render correctly
    expect(screen.getByText('Test Movie 1')).toBeInTheDocument()
    expect(screen.getByText('Test Movie 2')).toBeInTheDocument()
  })

  it('should use unique keys for skeleton cards', () => {
    const { container } = render(<MoviesList {...defaultProps} isLoading={true} />)
    
    const gridItems = container.querySelectorAll('.MuiGrid-item')
    expect(gridItems.length).toBe(20)
    
    // Each skeleton should have a unique key (tested by ensuring they all render)
    const skeletons = container.querySelectorAll('.MuiCard-root')
  ...
  it('should use movie id as key for movie cards', () => {
    ...
    // Should render one card per movie
    const movieCards = container.querySelectorAll('.MuiCard-root')
